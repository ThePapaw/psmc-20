...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.fuzzybritches_v4.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.fuzzybritches_v4.artwork').getAddonInfo('path'), 'resources', 'media', theme)

def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.fuzzybritches_v4.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.fuzzybritches_v4.artwork)')
...
def startupMaintenance():
    ...
    try:
        ...
        #xbmc.log('[ script.module.fuzzybritches_v4 ] Exception raised in. Error =' + str(e))   
        pass
