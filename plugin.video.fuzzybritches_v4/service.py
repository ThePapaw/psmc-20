...
def conversion():
    ...
    #curVersion = str(control.addon('script.module.fuzzybritches_v4').getAddonInfo('version'))
    curVersion = c.moduleversion
    version = c.pluginversion

    if os.path.exists(conversionFile):
        if c.has_silent_boot:
            c.log(f"You are fine, using module {curVersion} (video: {version}), Continuing...")
        else:
            control.infoDialog(f"You are fine, using module {curVersion} (video: {version}), Continuing...", sound=False)
    else:
        f_path = 'special://home/addons/script.fuzzybritchesv4.metadata'
...
def readProviders(scraper_path_fill, msg1, catnr):
    ...
    addon_path = xbmcvfs.translatePath('special://home/addons/plugin.video.fuzzybritches_v4')
    module_path = xbmcvfs.translatePath('special://home/addons/script.module.fuzzybritches_v4')


    xbmc.log(f"[ plugin.video.fuzzybritches_v4 ] service - checking {msg1} providers started")
    ...
    try:
        ...
        c.log(f"The Fuzzy Britches Service - Exception: \n {failure}")
        return
...
def syncTraktLibrary():
    control.execute('RunPlugin(plugin://plugin.video.fuzzybritches_v4/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute('RunPlugin(plugin://plugin.video.fuzzybritches_v4/?action=moviesToLibrarySilent&url=traktcollection')
...
def main():
    ...
    try:
        ...
        fum_ver = xbmcaddon.Addon('script.module.fuzzybritches_v4').getAddonInfo('version')
        updated = xbmcaddon.Addon('plugin.video.fuzzybritches_v4').getSetting('module_base')
        if not updated: 
            updated = '0'
        ...
        xbmcaddon.Addon('plugin.video.fuzzybritches_v4').setSetting('module_base', fum_ver)
